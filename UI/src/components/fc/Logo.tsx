...
export function Logo({
  ...
  return (
    <Link to="/" className={cn("flex items-center gap-3", className)}>
      <svg viewBox="0 0 24 24" className={cn(icon, forceDark ? "text-emerald-400" : "text-primary")} fill="currentColor" aria-hidden>
      ...
      <span className={cn("font-display font-extrabold leading-[0.95] tracking-tight", text)}>
        <span className={cn("block", forceDark ? "text-white" : "text-foreground")}>FANTASY</span>
        <span className={cn("block", forceDark ? "text-emerald-400" : "text-primary")}>CRICKET</span>
      </span>
    </Link>
  );
}
